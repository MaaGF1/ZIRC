(function(){
    ...
    const loadData = async () => {
      ...
      // [MODIFIED FOR OFFLINE MODE]
      // Changed fetching from githubusercontent to local relative path
      const loadStcFile = (file) => loadJsonFile(`../GFLData/${config.dataSource}/stc/${file}`);
      const loadCatchFile = (file) => loadJsonFile(`../GFLData/${config.dataSource}/catchdata/${file}`);
      const loadTextTable = (file) => loadJsonFile(`../GFLData/${config.langCode}/text/table/${file}`);
      const loadCnTextTable = (file) => loadJsonFile(`../GFLData/ch/text/table/${file}`);
      ...
      const loaders = {
        ...
        "Equip": loadStcFile(`equip.json`).then((result) => {
          Equip = result;
          // Note: original code lacked var declaration for Equip_map, keeping it as is to maintain your logic
